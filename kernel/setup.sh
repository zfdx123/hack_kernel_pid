...
test -d "$GKI_ROOT/hack_kernel_pid" || git clone https://github.com/zfdx123/hack_kernel_pid
cd "$GKI_ROOT/hack_kernel_pid"
git stash
...
echo "[+] GKI_ROOT: $GKI_ROOT"
echo "[+] Copy hack_kernel_pid driver to $DRIVER_DIR"
...
if test -d "$GKI_ROOT/common/drivers"; then
	if test -d "$GKI_ROOT/common/drivers/khack"; then
		...
		ln -sf "../hack_kernel_pid/kernel" "khack"
	fi
elif test -d "$GKI_ROOT/drivers"; then
...
grep -q "khack" "$DRIVER_MAKEFILE" || printf "obj-\$(CONFIG_HACK_HIDE_PID) += khack/\n" >> "$DRIVER_MAKEFILE"
